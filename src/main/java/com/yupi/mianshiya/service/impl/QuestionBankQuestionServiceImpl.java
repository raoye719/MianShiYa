...
import com.yupi.mianshiya.rabbitmq.QuestionBankAddMessage;
import com.yupi.mianshiya.rabbitmq.RabbitMQConfig;
...
//        }
    ...
    @Override
    public void batchAddQuestionsToBankRabbitMq(List<Long> questionIdList, long questionBankId, User loginUser) {
        // 参数校验
        ThrowUtils.throwIf(CollUtil.isEmpty(questionIdList), ErrorCode.PARAMS_ERROR, "题目列表不能为空");
        ThrowUtils.throwIf(questionBankId <= 0, ErrorCode.PARAMS_ERROR, "题库 id 非法");
        ThrowUtils.throwIf(loginUser == null, ErrorCode.NOT_LOGIN_ERROR);
        ...
        int batchSize = 1000;
        List<List<Long>> batchLists = Lists.partition(validQuestionIdList, batchSize);

        batchLists.forEach(batch -> {
            List<QuestionBankAddMessage> messages = batch.stream()
                    .map(questionId -> new QuestionBankAddMessage(questionId, questionBankId, loginUser.getId()))
                    .collect(Collectors.toList());
            ...
            MessagePostProcessor processor = m-> {
                ...
                m.getMessageProperties().setHeader("retry-count", 0); // 初始为0
                return m;
            };
...
//        // 发生消息到rabbitmq
//        validQuestionIdList.forEach(questionId -> {
//            QuestionBankAddMessage message = new QuestionBankAddMessage(
//                    questionId,
//                    questionBankId,
//                    loginUser.getId()
//            );
...
//            );
//        });
        log.info("已异步提交{}个题目到题库{}", validQuestionIdList.size(), questionBankId);
    }
